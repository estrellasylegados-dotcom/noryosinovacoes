...
- **Estado (03/09/2026) — Formulário V2 (respostas estruturadas):** ver a subseção "Formulário V2" abaixo. Coleta estruturada (chips/cards) no lugar de texto livre, `scoring_version` mantido em `"v1"` (sem recalibração), `form_version = "v2"`, colunas aditivas no Supabase, funil instrumentado (analytics sem PII). Commit `376a8c6`. **VALIDADO EM PRODUÇÃO em 03/09/2026** — migração aditiva aplicada no Supabase (5 colunas + 3 índices confirmados: `colunas_ok=5`, `indices_ok=3`); deploy na Hostinger + purge de CDN; envio real com linha completa (`form_version='v2'`, `scoring_version='v1'`, `score`/`maturidade_digital`/`classificacao`/`prioridade` preenchidos, `prazo`/`objetivo_principal`/`porte` corretos, `respostas` jsonb coerente, colunas legadas preenchidas); e-mail interno com o bloco "Respostas do lead (estruturado)"; leads antigos intactos (`form_version` NULL, nenhuma linha alterada); teste negativo (POST sem Turnstile → 403, contagem de linhas antes = depois, nenhum e-mail; `GET /api/diagnostico` → 405 com `Allow: POST`). **QUALITY GATE: APROVADO — VALIDADO EM PRODUÇÃO.**
...
- **Estado: VALIDADO EM PRODUÇÃO em 03/09/2026** (commit `376a8c6`). Migração aditiva aplicada, deploy + purge feitos, envio real conferido ponta a ponta (linha no Supabase com as colunas V2 + `scoring_version='v1'`, e-mail com bloco estruturado, leads antigos intactos, teste negativo Turnstile/405 ok). Próximo: observar leads reais (conversão + drop-off por etapa via `dataLayer`) antes de qualquer scoring v2.
...
- Nome do responsável: **confirmado "Rafael Viriato"** (03/09/2026); revisar o restante da copy de `/sobre` antes de dar destaque público
- Deploy e domínio: **feito** — deploy de produção na Hostinger Web Apps concluído, `noryosinovacoes.com.br` no ar (deploy automático a partir do `main`). Purge manual de CDN: **não foi mais necessário no deploy de 03/09/2026** (commit `a8b7bdc` apareceu na URL limpa sem purge; `x-hcdn-cache-status: DYNAMIC`, `s-maxage=30`) — confirmar em 1–2 deploys futuros
